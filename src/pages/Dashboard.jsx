...
import { Users, Calendar, AlertCircle, CheckCircle, ArrowUpRight, Clock, MoreVertical, RefreshCw } from "lucide-react";
...
const Dashboard = () => {
  ...
  // Data extracted to constant for reuse in Mobile/Desktop views
  const visitors = [
    { id: 1, name: "Dr. Alok Verma", role: "Guest Lecturer", type: "Event Guest", time: "09:30 AM", status: "Active", purpose: "AI Seminar" },
    { id: 2, name: "Rahul Singh", role: "Student Parent", type: "One-Day", time: "10:15 AM", status: "Checked Out", purpose: "Fee Payment" },
    { id: 3, name: "Priya Sharma", role: "Vendor", type: "Multi-Day", time: "11:00 AM", status: "Active", purpose: "Canteen Supply" },
    { id: 4, name: "Amit Kumar", role: "Alumni", type: "One-Day", time: "11:45 AM", status: "Active", purpose: "Library Visit" },
  ];
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6 lg:p-10 font-sans text-slate-900">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 opacity-0 animate-fade-in-up">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium text-sm sm:text-base">Welcome back, Admin. <span className="text-[#facc15] font-bold">4 new events</span> pending approval.</p>
        </div>
        
        {/* Live Status Pill */}
        <div className="mt-4 md:mt-0 flex items-center space-x-3 bg-white px-5 py-2.5 rounded-full border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all cursor-pointer group w-full md:w-auto justify-center md:justify-start">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-10">
        ...
        <StatCard
          ...
          value="03"
          trend="On Track"
          subtext="Tech Fest, Guest Lecture"
        ...
        <StatCard
          ...
          icon={<AlertCircle size={26} strokeWidth={1.5} />}
          delay={300}
        />
        <StatCard
          title="Passes Issued"
      ...
      >
        {/* Table Header - Flex column on mobile, row on desktop */}
        <div className="p-6 sm:p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-br from-white via-white to-blue-50/30">
          <div className="flex items-center gap-4 group cursor-pointer">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium">Real-time entry tracking</p>
            </div>
          </div>
          
          <button className="w-full md:w-auto group relative px-6 py-3 bg-[#0b1f3b] text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-900/20 hover:shadow-blue-900/40 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-100">
              {visitors.map((visitor) => (
                ...
                >
                  ...
                  <td className="px-6 py-5 text-sm text-slate-600 font-medium group-hover:text-slate-900">
                    {visitor.purpose}
                  </td>
                  
                  <td className="px-6 py-5">
                    <div className="flex items-center text-slate-500 font-mono text-sm bg-slate-50 px-2 py-1 rounded w-fit group-hover:bg-white group-hover:shadow-sm transition-all border border-transparent group-hover:border-slate-100">
                  ...
                  <td className="px-6 py-5 text-center">
                    <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold shadow-sm ${
                      ...
                      <span className={`w-2 h-2 rounded-full mr-2 ${
                        visitor.status === 'Active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'
                      }`}></span>
                      {visitor.status}
                    </div>
                  </td>
        ...
        <div className="md:hidden flex flex-col divide-y divide-slate-100">
           {visitors.map((visitor) => (
             <div key={visitor.id} className="p-5 bg-white hover:bg-slate-50 transition-colors">
               ...
               <div className="flex justify-between items-start mb-4">
                 <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      <p className="text-sm font-bold text-[#0b1f3b]">{visitor.name}</p>
                      <p className="text-xs text-slate-500 font-medium">{visitor.role}</p>
                    </div>
                 </div>
               ...
               <div className="flex flex-wrap gap-2 mb-4">
                 <span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${
                  ...
                  <span className="px-2.5 py-1 rounded-md text-xs font-medium text-slate-600 bg-slate-100 border border-slate-200">
                    {visitor.purpose}
                  </span>
               </div>
               ...
               <div className="flex justify-between items-center pt-2 border-t border-slate-50">
                  ...
                  <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                        visitor.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'
                      }`}></span>
                      {visitor.status}
                    </div>
               </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 text-center">
            <button className="text-xs font-bold text-[#0b1f3b] hover:text-blue-700 hover:underline transition-all">View All Entries</button>
        </div>
      </div>
    </div>
  );
