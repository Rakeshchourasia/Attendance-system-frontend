...
import { Camera, User, Phone, FileText, Calendar, MapPin, CreditCard, RefreshCw, CheckCircle, RotateCcw, Download, Printer, X } from 'lucide-react';
...
const InputField = ({ label, icon: Icon, type = "text", placeholder, ...props }) => (
  <div className="space-y-1.5 group">
    ...
    <div className="relative">
      ...
      <input
        type={type}
        className="block w-full pl-10 sm:pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm sm:text-base text-slate-900 font-medium placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-[#0b1f3b] transition-all duration-300 hover:border-blue-300"
...
const EventPass = ({ data, onReset }) => {
  ...
  // 1. Prepare the full data object for the QR Code
    ...
    <div className="w-full h-full flex flex-col items-center justify-center p-6 animate-fade-in-up">
      ...
      <div id="printable-pass" className="bg-white w-[320px] border border-slate-200 rounded-3xl shadow-2xl overflow-hidden relative">
        ...
        <div className="p-6 flex flex-col items-center text-center">
            ...
            {/* QR Code - UPDATED HERE */}
            <div className="p-2 border border-slate-100 rounded-lg bg-white shadow-sm mb-6">
               <QRCode 
                  value={qrCodeData} // <--- Now contains all 7 fields
                  size={120} 
                  fgColor="#0b1f3b"
                  level="M" // Medium error correction (good for dense data)
                />
            </div>

            {/* Visual Details Grid (Human Readable) */}
            <div className="w-full grid grid-cols-2 gap-3 text-left bg-slate-50 p-4 rounded-xl border border-slate-100">
               <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Pass ID</span>
               ...
               <div>
                  ...
                  <span className="text-xs font-bold text-slate-700 truncate">{data.hostName}</span>
               </div>
      ...
      <div className="mt-8 flex gap-4 w-full max-w-[320px]">
         <button 
           ...
           className="flex-1 bg-[#0b1f3b] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-blue-900 transition-colors shadow-lg"
         >
            <Printer size={18} /> Print
         </button>
         <button 
           onClick={onReset}
           className="flex-1 bg-white border border-slate-200 text-slate-600 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors shadow-sm"
         >
...
const VisitorRegistration = () => {
  ...
  const [generatedPass, setGeneratedPass] = useState(null); // Stores the API response
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/visitors', payload);
      
      // ON SUCCESS: Switch to Pass View
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6 lg:p-10 font-sans text-slate-900 flex justify-center items-start pt-6 sm:pt-10">
      ...
      <div className="w-full max-w-5xl">
        ...
        {/* Only show Header if NOT viewing the pass (optional, keeps UI clean) */}
        {!generatedPass && (
                ...
                <div className="md:w-1/3 bg-slate-50/80 p-5 sm:p-8 border-b md:border-b-0 md:border-r border-slate-100">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-1 gap-3">
                        ...
                        <button key={type} type="button" onClick={() => setVisitorType(type)}
                            ...
                            visitorType === type ? 'border-[#0b1f3b] bg-[#0b1f3b] shadow-md' : 'border-white bg-white hover:border-blue-200'
                            }`}>
                ...
                <div className="md:w-2/3 p-5 sm:p-10">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                            <InputField label="Purpose" name="purpose" value={formData.purpose} onChange={handleChange} icon={FileText} placeholder="Project meeting..." />
                        </div>
                        <InputField label="Host Name" name="hostName" value={formData.hostName} onChange={handleChange} icon={MapPin} placeholder="Host Name" />
